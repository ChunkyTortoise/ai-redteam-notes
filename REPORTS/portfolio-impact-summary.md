...
The portfolio is intentionally open-weight at this stage; see [open-weights-rationale.md](open-weights-rationale.md). The cross-scale question is no longer open in the speculative sense: the pre-registered F1 cell already shows the substrate effect strengthening from 8B to 70B (open-weight), so an open-weight result is a conservative lower bound rather than a weaker proxy. A frontier substrate replication on Claude (pre-registered as H10, with falsifier criteria for both replication and collapse) remains the next-phase confirmation; a confirmatory mitigation-tested grid (H10b-G) is in progress and gated. When those land, the results table gains a "generalizes to frontier?" column.
