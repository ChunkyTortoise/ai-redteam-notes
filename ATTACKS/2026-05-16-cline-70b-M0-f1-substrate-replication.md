...
The H7 falsification reframes the substrate finding. If the xml-dispatch substrate caused equivalent bypass rates at both scales, the mechanism would be substrate-only — alignment-independent. The 70B result inverts this: the larger model is more reliably exploited, achieving strict exfil where the 8B model only produced intent shifts and capability failures.
